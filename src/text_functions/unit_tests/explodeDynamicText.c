...
executeTest5(void)
{
    ...
    if (
        ...
        || (customStrlen(expectedText1, &errorNumber) != element1->getByteLength(&element1, &errorNumber, __FILE__, __LINE__))
        || (0 != customStrncmp(element1->getBytesPointer(&element1, &errorNumber), expectedText1, customStrlen(expectedText1, &errorNumber)))
        || (customStrlen(expectedText2, &errorNumber) != element2->getByteLength(&element2, &errorNumber, __FILE__, __LINE__))
...
executeTest6(void)
{
    ...
    if (
        ...
        || (customStrlen(expectedText2, &errorNumber) != element2->getByteLength(&element2, &errorNumber, __FILE__, __LINE__))
        || (0 != customStrncmp(element2->getBytesPointer(&element2, &errorNumber), expectedText2, customStrlen(expectedText2, &errorNumber)))
        || (customStrlen(expectedText3, &errorNumber) != element3->getByteLength(&element3, &errorNumber, __FILE__, __LINE__))
        || (0 != customStrncmp(element3->getBytesPointer(&element3, &errorNumber), expectedText3, customStrlen(expectedText3, &errorNumber)))
        || (customStrlen(expectedText4, &errorNumber) != element4->getByteLength(&element4, &errorNumber, __FILE__, __LINE__))
